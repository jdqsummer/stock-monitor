...
**目标:** 实现 L0-L3 四层记忆层级，支持分层检索、异步蒸馏管道、用户投资画像自动构建与更新，为 Agent 提供记忆上下文。

**架构:** Memory Store（CRUD 接口）→ 分层检索（level + category + 语义相似度）→ 异步蒸馏管道（分析对话 → LLM 提取 → 分层写入）→ L3 用户画像定时更新。

**技术栈:** Python 3.11+, SQLAlchemy 2.0 async, APScheduler, LLM (litellm), Pydantic v2

**依赖:** Plan-1（数据库 + User model）, Plan-4（分析 Agent + 数据采集 Agent 产出 L0 数据）
...
- L1/L2 异步蒸馏，不阻塞主流程（分析任务不等待蒸馏完成）
- L3 用户画像每周更新一次（定时任务）
- 记忆检索按 level 过滤，L1 限制 20 条，L2 限制 10 条，L3 全局 1 条
- 检索结果按置信度降序排列
- 蒸馏来源：分析 Agent 对话、用户与系统的交互记录
...
Plan-1: 核心平台 (DB + Auth + Config)
   ...
   │         │
   │         └──→ Plan-4: AI Agent 系统 (LangGraph + 分析 Agent + 数据采集 Agent + Harness)
   │                   │
   │                   └──→ Plan-3 的前端 Agent 页面（分析 UI）
   │

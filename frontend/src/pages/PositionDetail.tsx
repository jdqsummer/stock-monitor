    ...
    <div>
      ...
      <Card title={`${p.stock_name}（${p.stock_code}）`} style={{ marginTop: 12 }}>
        <Descriptions column={4} size="small" bordered>
          ...
          <Descriptions.Item label="成本价">{p.cost_price != null ? `¥${p.cost_price.toFixed(2)}` : '-'}</Descriptions.Item>
          <Descriptions.Item label="现价">{p.current_price != null ? `¥${p.current_price.toFixed(2)}` : '-'}</Descriptions.Item>

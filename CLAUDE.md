...
### 架构设计原则
- **分层清晰** — API 层只做路由，业务逻辑在 services/，AI Agent 在 agents/
- **模块化** — 每个模块独立，依赖注入，易于测试
- **可插拔** — LLM Provider、约束引擎、数据源
- **异步优先** — 所有 I/O 操作使用 async/await，数据库使用 AsyncSession
- **测试驱动** — TDD 开发流程，提交前必须通过全部测试
- **Plan-First** — 编码前必须出设计计划，用户确认后再编码
- **可观测性** — 日志、错误、约束检查结果必须记录，便于分析问题
- **可扩展性** — 设计考虑未来功能扩展，如多轮对话、记忆检索、蒸馏管道等
- **可维护性** — 代码风格统一，命名规范，文档齐全，便于团队协作和长期维护
